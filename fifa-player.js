// =====================================================
// FIFA LIVE - FIXED M3U8 REWRITER (COMPLETE VERSION)
// =====================================================
(function(){
  'use strict';
  ...
  // === STREAM LOADER - FIXED M3U8 REWRITER ===
  ...
  async function loadStreamViaProxy(originalUrl) {
    try {
      ...
      console.log('Fetching m3u8 via proxy:', proxiedM3u8Url);
      
      var response = await fetch(proxiedM3u8Url);
      if (!response.ok) throw new Error('HTTP ' + response.status);
      var m3u8Text = await response.text();
      console.log('M3U8 fetched, length:', m3u8Text.length);
      
      // Setup base URLs for resolving relative paths
      ...
      for (var i = 0; i < lines.length; i++) {
        ...
        // Skip empty lines and pure comments (no URI=)
        ...
        // Handle URL lines - RELATIVE, root-relative, or absolute
        var segUrl = null;
        ...
        } else if (line.indexOf('.ts') > -1 || line.indexOf('.m4s') > -1 || line.indexOf('.mp4') > -1) {
          // Relative URL (segment.ts) - COMMON CASE
          segUrl = baseUrl + line;
        } else {
      ...
      if (window.Hls && Hls.isSupported()) {
        ...
        hlsInstance.on(Hls.Events.MANIFEST_PARSED, function() {
          console.log('=== MANIFEST PARSED - Stream starting! ===');
          var p = video.play();
  ...
  function fetchData() {
    ...
    console.log('Fetching match data:', url);
    fetch(url)
      .then(function(r){ 
  ...
  function boot() {
    ...
    console.log('Version: FIXED M3U8 REWRITER');
    setupChannelTabs();
    fetchData();
