// =====================================================
// FIFA PLAYER v3.0-FIXED-2026 - PRODUCTION VERSION
// =====================================================
console.log('===== FIFA PLAYER v3.0-FIXED-2026 LOADED =====');
...
(function(){
  ...
  // === STREAM LOADER - SUPER ROBUST REWRITER ===
  function loadStream(channel) {
    ...
    if (!originalUrl) { console.warn('No stream URL'); return; }

    console.log('=== STREAM LOAD START v3.0 ===');
  ...
  async function loadStreamViaProxy(originalUrl) {
    try {
      ...
      console.log('M3U8 text length:', m3u8Text.length);
      
      var baseUrl = originalUrl.substring(0, originalUrl.lastIndexOf('/') + 1);
      var baseOrigin = new URL(originalUrl).origin;
      ...
      for (var i = 0; i < lines.length; i++) {
        ...
        // Skip empty and pure comment lines (no URI=)
        if (line === '' || (line.charAt(0) === '#' && line.indexOf('URI=') < 0)) {
          newLines.push(line);
          continue;
        }
        
        // Handle URI= attributes (encryption keys)
        if (line.indexOf('URI=') > -1) {
          var updated = line.replace(/URI="([^"]+)"/g, function(m, u) {
            ...
            return 'URI="' + CONFIG.proxyBase + encodeURIComponent(absUri) + '"';
          });
          newLines.push(updated);
          continue;
        }
        
        // Skip if this looks like a tag (starts with #)
        if (line.charAt(0) === '#') {
          newLines.push(line);
          continue;
        }
        
        // This is a URL line (could be relative, root-relative, or absolute)
        // ALWAYS rewrite through proxy
        var segUrl;
        if (line.indexOf('http') === 0 || line.indexOf('https') === 0) {
          ...
          // Root-relative URL
          segUrl = baseOrigin + line;
        } else {
          // Relative URL (most common for segments)
          segUrl = baseUrl + line;
        }
      ...
      console.log('=== M3U8 REWRITTEN v3.0 ===');
      console.log('Segments proxied:', rewrittenCount, '/', lines.length, 'total lines');
      
      // Verify rewrite - show first URL line
      for (var j = 0; j < newLines.length; j++) {
        if (newLines[j].indexOf('http') === 0 && newLines[j].indexOf('cors-proxy') > -1) {
          console.log('Sample rewritten URL:', newLines[j].substring(0, 100) + '...');
          break;
        }
      }
      ...
      if (window.Hls && Hls.isSupported()) {
        hlsInstance = new Hls({
          ...
          maxBufferLength: 30,
          enableWorker: false,
          // Add proxy config as fallback for absolute URLs
  ...
  function renderLiveMatch(data, match) {
    ...
    if (!match) {
      container.appendChild(el('div', { class: 'fifa-match-header' }, [
        ...
        el('div', { style: 'color:#9a9ab0;font-size:13px;font-weight:600;letter-spacing:1px;' }, ['See today matches below'])
      ]));
      return;
    }
